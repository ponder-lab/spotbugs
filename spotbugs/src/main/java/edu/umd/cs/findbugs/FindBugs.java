...
public abstract class FindBugs {
    ...
    public static void runMain(IFindBugsEngine findBugs, TextUICommandLine commandLine) throws IOException {
        ...
        if (verbose || commandLine.setExitCode()) {
            ...
            LOG.log(FINER, "Missing classes: {0}", missingClassCount);
            LOG.log(FINE, "Analysis errors: {0}", errorCount);
        }

        if (commandLine.setExitCode()) {
            int exitCode = 0;
            LOG.finer("Calculating exit code...");
            if (errorCount > 0) {
            ...
            if (missingClassCount > 0) {
                ...
                LOG.log(FINER, "Setting 'missing class' flag ({0})", ExitCodes.MISSING_CLASS_FLAG);
            }
            if (bugCount > 0) {
                exitCode |= ExitCodes.BUGS_FOUND_FLAG;
                LOG.log(FINER, "Setting 'bugs found' flag ({0})", ExitCodes.BUGS_FOUND_FLAG);
            }
            LOG.log(FINER, "Exit code set to: {0}", exitCode);
    ...
    public static void showSynopsis() {
        LOG.finest("Usage: findbugs [general options] -textui [command line options...] [jar/zip/class files, directories...]");
    }
